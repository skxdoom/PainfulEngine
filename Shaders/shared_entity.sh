...
//
// fs_entity and fs_entity_nm both include this; the second defines
// PAINFUL_ENTITY_NM, the palskinnedperpixel look (Skin.fxo's FXSkinBump): an
// OBJECT-space normal map, whose alpha masks a specular of (N.H)^10 x N.L.
...
void main()
{
	...
	float dirShadow = 1.0;
	if (u_vmParams.x > 0.5 && u_vmParams.y > 0.5)
