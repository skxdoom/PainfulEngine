...
The helper material this page names (`tools/*.ps1`, `Engine_API.md`,
`Engine_LuaAPI.md`, `exports/`, `Data_Extracted/`, PainKit) lives in
`PainfulEngineHelpers/`, outside this repository.
...
a disassembler. See `Engine_LuaAPI.md` (the editor's list was not kept),
...
the known-extension list must be COMPLETE (a censused list of all 62 shipped
...
else, silently, and `SaveGame:Load` ignores the result, so a `.World` in any
other format leaves the original with no entities and no
`SaveGame:AfterLoadEntities()`: script objects without their class methods
(`CSound.lua:173`), then a crash on handles that were never made. The port's
own data therefore goes in a side file, `<level>.World.pksv`.
...
and size). It is not always identity - `3x02_factory.mpk` has wall objects with a
0.83 scale in it - so it must not be applied. `MapObject::nodeTransform` keeps it
for the writer and `PainfulTools map <mpk> <filter>`, which prints it; nothing
else reads it.
...
**Object names carry semantics** (from the game's `Docs/Editor Docs/Pain Engine - MPK
...
Each material is a texture name plus the triangle run it covers (`firstIndex`,
`triangleCount`); there is no UV transform.

The header is found by a **self-validating search** backwards from the index
data (`Model::Load`): a candidate is accepted only if its runs tile the index
array exactly, in order, and the parse ends on the u32 index count. A wrong
guess is rejected rather than silently believed. The primary layout is name,
two u32, the normal-map string, the material list; lead-byte variants
12/8/16/4/0 are the fallback. A mesh whose header does not validate draws as
one slot over the whole index array.
...
The name is the leading string of the material header; the string two u32
later is the normal map (a path), not the name. A mesh name is never a path:
when a header does not validate, the last non-path string in the gap is used.

Check: `PainfulTools model pkw.pkmdl` names all 13 meshes. Four of the nine
names the Painkiller's alt fire hides (`polySurfaceShape47/44/29`,
`kolekShape`) are not in the model at all.
...
| `.rde` | 220 | Plain-text INI naming which bones are ragdoll limbs; its material values are one global constant ([`Hitboxes.md`](Hitboxes.md)). |
| `.hke` | 324 | The ragdoll itself: bodies, hulls and constraints ([`Physics.md`](Physics.md), "The binary .hke"). |
| `.dat` | 372 | Item mesh packs: string table, TOC and `WorldMesh` payloads (`Source/Assets/Dat.cpp`). |
...
- World collision is **built at load time from engine geometry**
  (`CreatePhysicsObjectFromMesh(..., WorldMesh*, ...)`,
  `CreatePhysicsObjectFromRagdoll(..., Model*, ...)`), not deserialised from Havok;
  ragdoll bodies and constraints are read from the `.hke` beside the model.
...
> Precedence is settled: `<name>2.pak`, `<name>1.pak`, `<name>.pak`, then the loose
> directory ("Numbered patch layering" above), and the port's `ReadFile` follows it, so a
> shipped name cannot be shadowed by a loose file; the mod loop above depends on `-lscripts`.
...
`Textures/Levels/<mapName>/`. `TextureCache::Resolve` does this (PainKit's
`TextureResolver` indexes
...
order. Over the 85 shipped maps, 12 lightmap-slot names on 10 maps
resolve only into another level and so draw white (`Okna_L_0000`,
...
PainKit (the external helper) includes a small **DDS reader** (DXT1/BC1, DXT3/BC2, DXT5/BC3 and
...
2. Import `Bin/Engine.dll` and run auto-analysis (the export table alone names
   ~2867 functions).
3. Window -> Script Manager -> Manage Script Directories -> add `PainfulEngineHelpers/tools`.
