...
a file from an older build (`Key = value`, comment blocks) is read and rewritten
in that style at the next start. A key this build does not know - renamed or
dropped - is left out of the rewrite, value and all; there is no migration, and
a renamed setting starts from its default.
