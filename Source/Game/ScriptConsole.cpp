...
// The Console natives. The struct is declared here rather than in
// ScriptEngine.h so that adding one touches only this file.
struct ConsoleNatives : ScriptNativesBase {
...
int ConsoleNatives::L_CONSOLE_IsActive(lua_State* L) {
...
int ConsoleNatives::L_CONSOLE_AddMessage(lua_State* L) {
...
int ConsoleNatives::L_CONSOLE_Print(lua_State* L) {
	From(L)->console_.AddMessage(luaL_optstring(L, 1, ""));
	return 0;
}

int ConsoleNatives::L_CONSOLE_SetCurrentText(lua_State* L) {
	From(L)->console_.SetCurrentText(luaL_optstring(L, 1, ""));
	return 0;
}

int ConsoleNatives::L_CONSOLE_GetCurrentText(lua_State* L) {
	lua_pushstring(L, From(L)->console_.currentText().c_str());
	return 1;
}

int ConsoleNatives::L_CONSOLE_GetCursorPos(lua_State* L) {
	lua_pushnumber(L, From(L)->console_.scrollPos());
	return 1;
}

// CONSOLE.SetFont(name, size)
int ConsoleNatives::L_CONSOLE_SetFont(lua_State* L) {
...
int ConsoleNatives::L_CONSOLE_SetMPMsgColor(lua_State* L) {
...
int ConsoleNatives::L_CONSOLE_SetMPMsgPosition(lua_State* L) {
	From(L)->console_.SetMessagePosition(float(luaL_optnumber(L, 1, 0)),
			float(luaL_optnumber(L, 2, 0)));
	return 0;
}

int ConsoleNatives::L_CONSOLE_SetMPMsgFont(lua_State* L) {
...
void BindConsole(ScriptEngine& engine, LuaHost& host) {
	const ScriptNative natives[] = {
		...
		{"CONSOLE", "Activate", ConsoleNatives::L_CONSOLE_Activate},
		{"CONSOLE", "IsActive", ConsoleNatives::L_CONSOLE_IsActive},
		{"CONSOLE", "AddMessage", ConsoleNatives::L_CONSOLE_AddMessage},
		{"CONSOLE", "Print", ConsoleNatives::L_CONSOLE_Print},
		{"CONSOLE", "SetCurrentText", ConsoleNatives::L_CONSOLE_SetCurrentText},
		{"CONSOLE", "GetCurrentText", ConsoleNatives::L_CONSOLE_GetCurrentText},
		{"CONSOLE", "GetCursorPos", ConsoleNatives::L_CONSOLE_GetCursorPos},
		{"CONSOLE", "SetFont", ConsoleNatives::L_CONSOLE_SetFont},
